...
function Invoke-TrainThenTest {
    ...
    if ((Test-Path $WeightPath) -and (-not $Force)) {
        Write-Host ("[SKIP] Weight exists, skip train+test: " + $WeightPath) -ForegroundColor Yellow
        return
    }
...
$cases = @(
    @{
        ...
        ClsWeighting = "none"
        CbMode = "1"
        WeightPath = "model_none_CLRS_b099.pth"
        OutTag = "model_none_CLRS_b099"
    },
    @{
        CaseName = "class_balanced_1-beta"
        ClsWeighting = "class_balanced"
        CbMode = "1-beta"
        WeightPath = "model_cb_CLRS_b099.pth"
        OutTag = "model_cb_CLRS_b099"
    },
    @{
        CaseName = "class_balanced_1"
        ClsWeighting = "class_balanced"
        CbMode = "1"
        WeightPath = "model_cb1_CLRS_b099.pth"
        OutTag = "model_cb1_CLRS_b099"
    }
)
...
Write-Host ("=" * 90)
Write-Host "CLRS(beta=0.99): all three loss setups finished train+test (IF=$ImbFactor, bits=$HashBits)." -ForegroundColor Magenta
